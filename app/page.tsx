...
export default function SolarXCommissionCalculator() {
  ...
  const coachTip = (() => {
    if (saleKind === "tpo")
      return "TPO/PPA selected: Only base payout varies with monthly deals. PPW & system bonuses are not applied.";
    const kw = watts / 1000;
  ...
  // Active segmented toggle styles
  const Segmented = ({ value, onChange }: { value: "loan" | "tpo"; onChange: (v: "loan" | "tpo") => void }) => (
    <div
      className={`inline-flex rounded-xl p-1 ${isLight ? "bg-zinc-200" : "bg-zinc-800"}`}
      role="tablist"
    ...
    >
      {(["loan", "tpo"] as const).map((k) => (
        <button
          ...
          aria-label={k === "loan" ? "Loan or Cash" : "TPO or PPA (base only)"}
          onClick={() => onChange(k)}
  ...
  const Box = ({
    ...
    const glowStyle: CSSProperties | undefined = highlight
      ? {
          boxShadow: `0 0 0 2px rgba(${glowRGB}, ${isLight ? 0.35 : 0.75}), 0 0 40px 8px rgba(${glowRGB}, ${
            isLight ? 0.18 : 0.35
          })`,
          position: "relative",
        }
  ...
  return (
    <div className={`min-h-screen w-full ${pageBg} ${textBase} p-6`}>
      ...
      <div className="mx-auto max-w-6xl grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* MAIN CALCULATOR */}
        <Card
          className={`rounded-2xl border ${cardBg} shadow-[0_0_60px_-20px_rgba(100,150,255,0.15)] lg:col-span-2 relative overflow-hidden`}
        >
          <CardHeader className="pb-2">
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                >
                  <button
                    ...
                    className={`px-3 py-1 rounded-lg text-sm ${isLight ? primaryBtn() : subtleBtn()}`}
                  >
                    Light
                  </button>
                  <button
                ...
                >
                  {(["none", "sunset", "bw"] as Accent[]).map((a) => (
                    <button
                      ...
                      onClick={() => setAccent(a)}
                      className={`px-3 py-1 rounded-lg text-sm ${accent === a ? primaryBtn() : subtleBtn()}`}
                    >
            ...
            <p className={`${mutedText} text-sm`}>
              Type values or use the sliders. System size is in <span className="font-semibold">watts</span> (e.g., 8000
              for 8kW).
            </p>
          </CardHeader>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-3">
                  <Input
                    ...
                    className={`${inputBg} ${inputPh} disabled:opacity-60 disabled:cursor-not-allowed`}
                  />
                  <Button
                    className={subtleBtn()}
                    onClick={() => {
            ...
            {result && (
              <section className="relative">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  ...
                  <Box
                    label="System Bonus"
                    value={result.bigSystemBonus}
                    color={isLight ? "text-rose-700" : "text-red-300"}
                  />
                  <Box
                    label="Total Payout"
                    value={result.total}
                    color={isLight ? "text-sky-700" : "text-cyan-300"}
                    highlight={spotlight}
                  />
                </div>
            ...
            <div>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className={`rounded-xl border ${panelBg} p-3`}>
                  ...
                  <div className={`text-3xl font-bold ${isLight ? "text-emerald-700" : "text-emerald-300"}`}>
                    ${currency(myEarnings)}
                  </div>
                </div>
              </div>
            ...
            <div>
              ...
              <div className="mb-2 flex items-center gap-2">
                <Input
                  ...
                  className={`${inputBg} ${inputPh}`}
                />
                <Button className={subtleBtn("shrink-0")} onClick={() => setSavedQuery("")}>
                  Clear
                </Button>
              </div>
              <div className="space-y-2">
                ...
                {saved.length > 0 && filteredSaved.length === 0 && (
                  <div className={`text-sm ${mutedText}`}>No matches. Try a different search.</div>
                )}
                {filteredSaved.map((s) => (
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <div>
                        ...
                        <div className={`text-xs ${mutedText}`}>
                          {new Date(s.createdAt).toLocaleString()} • {s.watts} W • PPW {s.ppw.toFixed(2)} • $
                          {currency(s.total)} • {s.saleKind?.toUpperCase() || "LOAN"}
                        </div>
                      </div>
