...
const CONFETTI_COLORS = ["#22c55e", "#10b981", "#60a5fa", "#38bdf8", "#fde047", "#fca5a5", "#f472b6", "#a78bfa"] as const;
...
// Accent palettes that layer on top of the mode
// per request: keep "sunset" and a "black & white" (bw) palette
// (additional palettes can be added easily later)
 type Accent = "none" | "sunset" | "bw";

const ACCENTS: Record<Accent, {
  titleGradientLight: string; // gradient text for light mode
  titleGradientDark: string;  // gradient text for dark mode
  spotRGB: string;            // for subtle glows
  primaryBtn: string;         // extra classes applied to primary button
...
export default function SolarXCommissionCalculator() {
  // Simple client-side auth (hardcoded per request)
  ...
  const handleLogin = (e?: React.FormEvent) => {
    ...
    if (u === AUTH.username && p === AUTH.password) {
      try { localStorage.setItem("sx_authed", "1"); } catch {}
      setAuthed(true);
      setAuthErr("");
    } else {
  ...
  const handleLogout = () => {
    try { localStorage.removeItem("sx_authed"); } catch {}
    setAuthed(false);
    setU(""); setP("");
  };

  // Mode & accent
  const [mode, setMode] = useState<Mode>(() => (typeof window !== "undefined" ? ((localStorage.getItem("sx_mode") as Mode) || "light") : "light"));
  const [accent, setAccent] = useState<Accent>(() => (typeof window !== "undefined" ? ((localStorage.getItem("sx_accent") as Accent) || "none") : "none"));

  // Sale kind
  type SaleKind = "loan" | "tpo";
  const [saleKind, setSaleKind] = useState<SaleKind>(() => (typeof window !== "undefined" ? ((localStorage.getItem("sx_saleKind") as SaleKind) || "loan") : "loan"));
  ...
  useEffect(() => { try { localStorage.setItem("sx_mode", mode); } catch {} }, [mode]);
  useEffect(() => { try { localStorage.setItem("sx_accent", accent); } catch {} }, [accent]);
  useEffect(() => { try { localStorage.setItem("sx_saleKind", saleKind); } catch {} }, [saleKind]);
  ...
  useEffect(() => { setResult(compute()); }, [deals, ppw, watts, saleKind]);

  const persistSaved = (next: SavedDeal[]) => { setSaved(next); try { localStorage.setItem("sx_savedDeals", JSON.stringify(next)); } catch {} };
  ...
  const subtleBtn = (extra = "") => `${isLight ? "bg-zinc-100 hover:bg-zinc-200 text-zinc-900" : "bg-zinc-800 hover:bg-zinc-700 text-zinc-100"} ${extra}`;
  ...
  const coachTip = (() => {
    ...
    if (kw < 10) nextKw = 10; else if (kw < 20) nextKw = Math.floor(kw) === kw ? kw + 1 : Math.ceil(kw); else nextKw = null;
  ...
  const saveCurrent = () => {
    ...
    const item: SavedDeal = { id: `${Date.now()}`, name, deals, ppw, watts, total: next.total, createdAt: Date.now(), saleKind };
    persistSaved([item, ...saved].slice(0, 12));
  };
  const loadDeal = (id: string) => {
    const item = saved.find((s) => s.id === id);
    if (!item) return;
    setDeals(item.deals); setPpw(item.ppw); setPpwInput(item.ppw.toFixed(2)); setWatts(item.watts); setWattsInput(String(item.watts));
  ...
  const renameDeal = (id: string) => { const name = prompt("New name for this deal?"); if (!name) return; persistSaved(saved.map((s) => (s.id === id ? { ...s, name } : s))); };

  // Active segmented toggle styles (fixes visibility of which one is selected)
  const Segmented = ({ value, onChange }: { value: SaleKind; onChange: (v: SaleKind) => void }) => (
    <div className={`inline-flex rounded-xl p-1 ${isLight ? "bg-zinc-200" : "bg-zinc-800"}`} role="tablist" aria-label="Sale type">
      {(["loan", "tpo"] as SaleKind[]).map((k) => (
        <button
          key={k}
          role="tab"
  ...
  const Box = ({ label, value, color, highlight }: { label: string; value: number | string; color: string; highlight?: boolean }) => {
    ...
    const glowStyle: CSSProperties | undefined = highlight ? { boxShadow: `0 0 0 2px rgba(${glowRGB}, ${isLight ? 0.35 : 0.75}), 0 0 40px 8px rgba(${glowRGB}, ${isLight ? 0.18 : 0.35})`, position: "relative" } : undefined;
    return (
      <div className={`rounded-xl border ${panelBg} p-4 flex flex-col justify-between min-h-[100px]`} style={glowStyle}>
        <p className={`${mutedText} text-sm truncate`} title={String(value)}>{label}</p>
  ...
  const RecomputeButton = () => (
    <Button
      onClick={() => {
        const next = compute();
        setResult(next); dropCash(); triggerConfettiIfNeeded(next.total);
        if (next?.total) {
          const newDeals = myDeals + 1; const newEarn = myEarnings + next.total;
          setMyDeals(newDeals); setMyEarnings(newEarn);
          try { localStorage.setItem("sx_myDeals", String(newDeals)); localStorage.setItem("sx_myEarn", String(newEarn)); } catch {}
        }
      }}
  ...
  const filteredSaved = saved.filter((s) => {
    const q = savedQuery.trim().toLowerCase(); if (!q) return true;
  ...
  return (
    <div className={`min-h-screen w-full ${pageBg} ${textBase} p-6`}>
      ...
      {bursts.map((pieces, idx) => (<ConfettiBurst key={idx} pieces={pieces} />))}
        ...
        <Card className={`rounded-2xl border ${cardBg} shadow-[0_0_60px_-20px_rgba(100,150,255,0.15)] lg:col-span-2 relative overflow-hidden`}>
          <CardHeader className="pb-2">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
              <CardTitle>
                <div className={`text-4xl md:text-5xl font-extrabold tracking-tight bg-gradient-to-r ${titleGradient} bg-clip-text text-transparent`} style={titleGlow}>
              ...
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <div className={`inline-flex rounded-xl p-1 ${isLight ? "bg-zinc-200" : "bg-zinc-800"}`} role="tablist" aria-label="Color mode">
                  <button aria-selected={isLight} onClick={() => setMode("light")} className={`px-3 py-1 rounded-lg text-sm ${isLight ? primaryBtn() : subtleBtn()}`}>Light</button>
                  <button aria-selected={!isLight} onClick={() => setMode("dark")} className={`px-3 py-1 rounded-lg text-sm ${!isLight ? primaryBtn() : subtleBtn()}`}>Dark</button>
                </div>

                {/* Accent toggle (optional) */}
                <div className={`inline-flex rounded-xl p-1 ${isLight ? "bg-zinc-200" : "bg-zinc-800"}`} role="tablist" aria-label="Accent">
                  {(["none", "sunset", "bw"] as Accent[]).map((a) => (
                    <button key={a} aria-selected={accent === a} onClick={() => setAccent(a)} className={`px-3 py-1 rounded-lg text-sm ${accent === a ? primaryBtn() : subtleBtn()}`}>{a === "none" ? "Neutral" : a === "bw" ? "B&W" : "Sunset"}</button>
                  ))}
                </div>

                {/* Sale type toggle (segmented, clearly shows active) */}
                <Segmented value={saleKind} onChange={setSaleKind} />

                {/* Logout */}
                <Button className={subtleBtn()} onClick={handleLogout} title="Sign out">Logout</Button>
              </div>
            </div>

            <p className={`${mutedText} text-sm`}>Type values or use the sliders. System size is in <span className="font-semibold">watts</span> (e.g., 8000 for 8kW).</p>
          </CardHeader>
            ...
            <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-3">
                  <Input type="number" inputMode="numeric" value={deals} onChange={(e) => setDeals(Math.max(0, Math.min(50, Number(e.target.value))))} className={`${inputBg}`} />
                  <Button className={subtleBtn()} onClick={() => setDeals(0)}>Clear</Button>
                </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-3">
                  <Input type="text" inputMode="decimal" value={ppwInput} onChange={(e) => setPpwInput(e.target.value)} onBlur={commitPpw} disabled={saleKind === "tpo"} className={`${inputBg} ${inputPh} disabled:opacity-60 disabled:cursor-not-allowed`} />
                  <Button className={subtleBtn()} onClick={() => { setPpw(2.4); setPpwInput("2.40"); }} disabled={saleKind === "tpo"}>Clear</Button>
                </div>
                <Slider value={[ppw]} onValueChange={(v) => { const val = Number(v[0].toFixed(2)); setPpw(val); setPpwInput(val.toFixed(2)); }} min={2.4} max={4.5} step={0.01} disabled={saleKind === "tpo"} />
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-3">
                  <Input type="text" inputMode="numeric" value={wattsInput} onChange={(e) => setWattsInput(e.target.value)} onBlur={commitWatts} disabled={saleKind === "tpo"} className={`${inputBg} ${inputPh} disabled:opacity-60 disabled:cursor-not-allowed`} />
                  <Button className={subtleBtn()} onClick={() => { setWatts(0); setWattsInput("0"); }} disabled={saleKind === "tpo"}>Clear</Button>
                </div>
                <Slider value={[watts]} onValueChange={(v) => { const val = Math.round(v[0]); setWatts(val); setWattsInput(String(val)); }} min={4000} max={30000} step={100} disabled={saleKind === "tpo"} />
              </div>
            </section>
            ...
            {result && (
              <section className="relative">
                ...
                <div className="mt-4 flex flex-wrap items-center gap-3">
                  <RecomputeButton />
                  <Button className={subtleBtn()} onClick={() => { clearCash(); setBursts([]); }}>Clear Cash</Button>
                </div>
              </section>
            )}
          ...
          <CardContent className="space-y-6">
            ...
            <div>
              ...
              <div className="mt-3 flex gap-2">
                <Button className={subtleBtn()} onClick={() => { setMyDeals(0); setMyEarnings(0); try { localStorage.removeItem("sx_myDeals"); localStorage.removeItem("sx_myEarn"); } catch {} }}>Reset</Button>
                <Button className={subtleBtn()} onClick={saveCurrent}>Save Current</Button>
              </div>
            </div>
                  ...
                  <div key={s.id} className={`rounded-lg border ${panelBg} p-3`}>
                    <div className="flex items-center justify-between gap-2">
                      <div>
                        ...
                        <div className={`text-xs ${mutedText}`}>{new Date(s.createdAt).toLocaleString()} • {s.watts} W • PPW {s.ppw.toFixed(2)} • ${currency(s.total)} • {s.saleKind?.toUpperCase() || "LOAN"}</div>
                      </div>
                      <div className="flex gap-2">
                        <Button size="sm" className={subtleBtn()} onClick={() => loadDeal(s.id)}>Load</Button>
                        <Button size="sm" className={subtleBtn()} onClick={() => renameDeal(s.id)}>Rename</Button>
                        <Button size="sm" className={subtleBtn()} onClick={() => deleteDeal(s.id)}>Delete</Button>
                      </div>
                    </div>
                  </div>
                ))}
...
// ----------------------
if (typeof window !== "undefined" && !(window as any).__SX_TESTS_RAN__) {
  (window as any).__SX_TESTS_RAN__ = true;
  (function runTests() {
    const approxEq = (a: number, b: number) => Math.abs(a - b) < 1e-6;
    const t1 = computePayout(1, 3.0, 8000); console.assert(t1.base === 1800 && t1.ppwBonus === 400 && t1.bigSystemBonus === 0 && t1.total === 2200, "Test 1 failed", t1);
    const t2 = computePayout(7, 3.6, 14000); console.assert(t2.base === 2500 && t2.ppwBonus === 1400 && t2.bigSystemBonus === 400 && t2.total === 4300, "Test 2 failed", t2);
    const t3 = computePayout(0, 2.85, 12000); console.assert(t3.base === 0 && approxEq(t3.ppwBonus, 300) && t3.bigSystemBonus === 300 && t3.total === 600, "Test 3 failed", t3);
    ...
    const t7 = computePayout(0, 3.0, 13500); console.assert(t7.base === 0 && t7.ppwBonus === 675 && t7.bigSystemBonus === 375 && t7.total === 1050, "Test 7 failed", t7);
    const t8 = computePayout(1, 2.8, 20000); console.assert(t8.base === 1800 && t8.ppwBonus === 500 && t8.bigSystemBonus === 700 && t8.total === 3000, "Test 8 failed", t8);
    const t9 = computePayout(4, 3.2, 0); console.assert(t9.base === 2200 && t9.ppwBonus === 0 && t9.bigSystemBonus === 0 && t9.total === 2200, "Test 9 failed", t9);
    const t10 = computePayout(7, 3.5, 30000); console.assert(t10.base === 2500 && t10.ppwBonus === 2250 && t10.bigSystemBonus === 800 && t10.total === 5550, "Test 10 failed", t10);
    const t11 = computePayout(0, 3.6, 17500); console.assert(t11.bigSystemBonus === 600, "Test 11 failed", t11);
    const t12 = computePayout(1, 3.21, 15000); console.assert(t12.base === 1800 && t12.ppwBonus === 1125 && t12.bigSystemBonus === 450 && t12.total === 3375, "Test 12 failed", t12);
    const t13 = computePayout(0, 3.0, 9900); console.assert(t13.bigSystemBonus === 0, "Test 13 failed", t13);
    const t14 = computePayout(0, 3.0, 21000); console.assert(t14.bigSystemBonus === 700, "Test 14 failed", t14);
    const t15 = computePayoutTpo(7, 3.6, 20000); console.assert(t15.base === 2500 && t15.ppwBonus === 0 && t15.bigSystemBonus === 0 && t15.total === 2500, "Test 15 (TPO) failed", t15);
    const t16 = computePayoutTpo(0, 3.6, 20000); console.assert(t16.base === 0 && t16.ppwBonus === 0 && t16.bigSystemBonus === 0 && t16.total === 0, "Test 16 (TPO) failed", t16);
  })();
}
